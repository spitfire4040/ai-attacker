...
def main(argv):
    arr = os.listdir('/home/jay/Desktop/Chime_Doorbell')
    path = '/home/jay/Desktop/Chime_Doorbell/'
    for x in range(0, len(arr)):
        for y in range(x, len(arr)):
            if arr[x] != arr[y]:
                with open(path + arr[x], 'rb') as f1:
                    with open(path + arr[y], 'rb') as f2:
